...
class YandexOCRAsync:
    def __init__(self, iam_token: str, folder_id: str) -> None:
        ...
        self.headers = {"Authorization": f"Bearer {iam_token}", "x-folder-id": folder_id}
    ...
    def split_pdf(self, input_path: str, pages_per_chunk: int = 1) -> list[tuple[str, int]]:
        """
    ...
    async def recognize_pdf(self, session: aiohttp.ClientSession, file_path: str) -> str | None:
        """
    ...
    async def process_chunk(self, session: aiohttp.ClientSession, chunk_path: str, page_number: int, semaphore: asyncio.Semaphore) -> dict[Any, Any] | None:
        async with semaphore:
            ...
            return {"page": page_number + 1, "text": self.extract_text_from_result(ocr_result)}

    async def process_pdf(self, input_path: str, max_concurrent: int = 10) -> list[dict[Any, Any]]:
